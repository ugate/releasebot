...
exports.cloneAndSetCommit = cloneAndSetCommit;
...
function genCommit(env) {
	...
	var c = new Commit(env.releaseVersionDefaultLabel,
			env.releaseVersionRegExp, env.bumpVersionRegExp, cm, pver, true,
			env.gitCliSubstitute, ch, env.pkgPath, env.buildDir, br, rs, un,
			rn, env.gitToken, env.npmToken);
	cloneAndSetCommit(c);
	return c;
}
 ...
 */
function Commit(relLbl, relRx, bumpRx, cmo, pver, nver, gitCliSubstitute, ch,
		pkgPath, buildDir, branch, slug, username, reponame, gitToken, npmToken) {
	...
	this.prev = pver instanceof Commit ? pver
			: typeof pver === 'string' ? new Commit(relLbl, relRx, bumpRx,
					(self.versionLabel || relLbl) + ' ' + self.versionType
							+ pver, null, self) : {
				version : '0.0.0',
				versionMatch : []
			};
	...
	this.next = nver === true && this.version ? new Commit(relLbl, relRx,
			bumpRx, {
				matcher : bumpRx,
				message : cm,
				altMatcher : relRx,
