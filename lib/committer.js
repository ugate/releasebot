 ...
 */
function getEnv(ns) {
	return rbot.config(getEnvNS(ns));
}
...
/**
 * Gets the namespace used for a {Commit} environment
 * 
 * @param ns
 *            the optional namespace to use
 * @returns the namespace
 */
function getEnvNS(ns) {
	return ns ? envNS + (ns === envNS ? '' : '.' + ns) : envNS;
}
