...
function genCommit(env, msg, ns, prevVer) {
	...
	function smv(cm, rx, pv) {
		...
		return cm.replace(rx, function relSemver(m, lbl, typ, rel) {
			return lbl + typ + semver.inc(pv, rel);
		});
	}
	if (!br) {
...
function Commit(relLbl, relType, relRx, bumpRx, cmo, pver, nver,
	...
	this.versionPrereleaseChar = rv.length > 9 ? rv[9] : '';
	...
	var versionSuffix = verSuffix(10);
	...
	function verSuffix(i) {
		...
		v = v.replace(regexVerCurrBumpNum, function verCurrRepl(m, cg, bg, ng,
			...
			if (cg) {
				...
				// only replace when a version separator is adjacent to the bump
				// match or the bump match is the only contents of the string
				if (bmpAll || mdi !== off) {
					self.versionBumpedIndices.push(++si);
					return (pvers && pvers.length > vi ? +pvers[vi] : 0)
							+ m.length;
	...
	function vver(pt, inc) {
		return (pt ? vv(2) : '')
				...
				+ vv(7)
				+ vv(8, self.versionPatch, inc && !self.versionPrereleaseChar)
				+ vv(9)
				+ vv(10, self.versionPrerelease, inc
						&& self.versionPrereleaseChar)
				+ vv(11, self.versionMetadata);
	}
