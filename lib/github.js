 ...
 */
exports.releaseAndUploadAsset = function releaseAndUploadAsset(assets, regexLines, commit, name, desc, options,
		rollCall, fcb, cb) {
	var authToken = typeof commit.gitToken === 'function' ? commit.gitToken() : commit.gitToken;
	if (!authToken) {
	...
	function nextAsset() {
		assetIndex++;
		var a = {
			item : Array.isArray(assets) && assetIndex < assets.length ? assets[assetIndex] : null
		};
	...
	json[releasePreFlag] = typeof commit.versionPrereleaseChar === 'string' && commit.versionPrereleaseChar.length > 0;
	...
	function postRelease() {
		rbot.log.info('Posting the following to ' + opts.hostname + releasePath);
		if (rbot.log.verboseEnabled) {
			rbot.log.verbose(util.inspect(json, {
				colors : true
			}));
		}
		var resData = '';
		...
		var req = https.request(opts, function(r) {
			...
			res.on('data', function(chunk) {
				resData += chunk;
				rbot.log.verbose('Receiving post release chunked data');
			});
			res.on('end', function() {
				if (httpSuccessCodes.indexOf(res.statusCode) >= 0) {
					rbot.log.verbose('Received post release data');
					rollCall.then(postReleaseEnd).resume();
				} else {
					rollCall.error(
							'Release post failed with HTTP status: ' + res.statusCode + ' data: '
		...
		function postReleaseEnd() {
			...
			rl = success ? chk(JSON.parse(resData.replace(regexLines, ' '))) : null;
			if (rbot.log.verboseEnabled) {
				rbot.log.verbose(util.inspect(rl, {
					colors : true
				}));
			}
			if (rl && rl[releaseTagName] === commit.versionTag) {
				...
				rollCall.error('No tag found for ' + commit.versionTag + ' in ' + util.inspect(rl, {
					colors : true
				}) + ' HTTP Status: ' + res.statusCode + ' Response: \n' + resData).then(cb).resume();
			}
		}
	}

	function postReleaseAsset() {
		rbot.log.info('Uploading "' + asset.item.path + '" release asset for ' + commit.versionTag + ' via '
				+ options.gitHostname);
		opts.method = 'POST';
		...
		opts.path = opts.path.replace(regexParam, '$1=' + (asset.item.name || commit.versionTag));
		...
		var req = https.request(opts, function(r) {
			...
			res.on('data', function(chunk) {
				resData += chunk;
				rbot.log.verbose('Receiving upload response');
			});
			res.on('end', function() {
				rbot.log.verbose('Received upload response');
				rollCall.then(postRleaseAssetEnd).resume();
			});
			rbot.log.info('Waiting for response');
		});
		req.on('error', function(e) {
			resError = e;
			rollCall.then(postRleaseAssetEnd).resume();
		...
		function postRleaseAssetEnd() {
			if (resError) {
				...
				if (ajson && ajson.state !== 'uploaded') {
					var msg = 'Asset upload failed with state: ' + ajson.state + ' for ' + util.inspect(ajson, {
						colors : true
					});
					rollCall.error(msg);
				} else {
					var durl = 'https://' + options.gitHostname + '.com/' + commit.username + '/' + commit.reponame
							+ '/releases/download/' + commit.versionTag + '/' + ajson[releaseName];
					// make asset avaliable via commit
					commit.releaseAssets.push({
						asset : ajson,
						downloadUrl : durl
					});
					rbot.log.info('Asset ID ' + ajson[releaseAssetId] + ' successfully ' + ajson.state + ' for '
							+ asset.item.name + ' ' + asset.item.path + ' (downloadable at: ' + durl + ')');
					if (rbot.log.verboseEnabled) {
						rbot.log.verbose(util.inspect(ajson, {
							colors : true
						}));
					}
				}
				...
				rollCall.error('Asset upload failed with HTTP status: ' + res.statusCode + ' data: ' + dstr);
			}
	...
	function postReleaseRollback() {
		...
		try {
			...
			rbot.log.info('Rolling back ' + commit.versionTag + ' release via ' + options.gitHostname + ' '
					+ opts.method + ' ' + opts.path);
			var rreq = https.request(opts, function(r) {
				res = r;
				res.on('data', function(chunk) {
					rbot.log.verbose('Receiving release rollback data');
					rrdata += chunk;
				});
			...
			rollCall.error('Failed to request rollback for release ID ' + commit.releaseId, e);
		}
		function postReleaseRollbackEnd() {
			try {
				var msg = 'Release rollback for release ID: ' + commit.releaseId;
				if (httpSuccessCodes.indexOf(res.statusCode) >= 0) {
					rbot.log.info(msg + ' complete');
					rbot.log.verbose(rrdata);
				} else {
