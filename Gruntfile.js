...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		copy : {
			dist : {
				expand : true,
				src : [ '{tasks,lib,test,css,img}/**/*.{js,md,css,svg,png}', 'LICENSE', 'README.md', pkg.main ],
				dest : distPath
			}
		},
		...
		releasebot : {
			options : {
				...
				distBranchUpdateFunction : function(contents, path, commit) {
					var zx = /zip/i;
					var tx = /tar/i;
					...
					if (zip || tar) {
						...
						// asset download URL
						contents = contents.replace(regexDistUrl, function(m, prefix, url, suffix) {
							var au = prefix + assetUrl(url) + suffix;
							grunt.log.writeln('Replacing "' + m + '" with "' + au + '"');
							return au;
						});
					}
					function assetUrl(url) {
						return zx.test(url) && zip ? zip.downloadUrl : tx.test(url) && tar ? tar.downloadUrl : '';
					}
					function asset(a, rx) {
						for (var i = 0; i < a.length; i++) {
							if (a[i] && rx.test(a[i].asset.content_type)) {
								return a[i];
							}
						}
						return null;
	...
	function writeHtml() {
		...
		var rmhtml = '<!DOCTYPE html><html><head>'
				+ '<meta http-equiv="content-type" content="text/html;charset=utf-8" />' + '<title>releasebot</title>'
				+ '<link href="css/index.css" rel="stylesheet" media="screen" />' + '</head><body>' + marked(rmmd)
				+ '</body></html>';
	...
	grunt.log.writeflags = function(obj, prefix) {
		var m = null;
		if (typeof obj === 'object' && obj && (m = Object.getOwnPropertyNames(obj).join(',').match(regexSuppressWrite))
				&& m.length > 0) {
