 ...
 */
exports.releaseAndUploadAsset = function releaseAndUploadAsset(assets,
	...
	function postReleaseAsset() {
		grunt.log.writeln('Uploading "' + asset.item.path
				...
				+ options.gitHostname);
		opts.method = 'POST';
		opts.path = rl[gitHubReleaseUploadUrl].replace(regexHost,
				function(m, h) {
					opts.hostname = h;
					return '/';
				});
		...
		function postRleaseAssetEnd() {
			if (resError) {
				...
				if (ajson && ajson.state !== 'uploaded') {
					...
					grunt.log.writeln('Asset ID ' + ajson[gitHubReleaseAssetId]
